# Claude Code Statusline - Masterpiece Edition
...
- **Ambiente Python**: Rileva automaticamente conda, venv, poetry
- **Barre Token**: IN (input), OUT (output), Skills, MCP, Total con colori pastello
- **MCP Servers**: Conteggio, stato di salute, lista dettagliata
- **Skills & Plugins**: Conteggio delle skills e plugins attivi
- **Warning automatici**: Alert visivi quando il contesto si riempie (70%, 85%)
- **Profilo corrente**: Supporto per profili (base, ios, web, python, full)

## Requisiti

- [Claude Code](https://claude.ai/code) installato
- `jq` per il parsing JSON (`brew install jq` su macOS)
- Bash 4+ (default su macOS/Linux)
...
**Importante**: Sostituisci `YOUR_USERNAME` con il tuo username reale, oppure usa:
...
Chiudi e riapri Claude Code per applicare le modifiche.

## Configurazione Avanzata

### Settings.json Completo (Esempio)
...
La statusline supporta profili colorati. Crea il file `~/.claude/.current_profile`:

```bash
echo "ios" > ~/.claude/.current_profile
```

Profili disponibili:
- `base` - Grigio
- `ios` - Ciano (per sviluppo iOS)
- `web` - Magenta (per sviluppo web)
- `python` - Giallo (per progetti Python)
- `full` - Rosso (tutte le funzionalità)

## MCP Servers (Opzionale)

Per aggiungere MCP servers, modifica la sezione `mcpServers` in `settings.json`:
...
La statusline mostrerà automaticamente:
- Numero di MCP servers configurati
- Stato di salute (verde = OK, rosso = non raggiungibile)
- Lista compatta con tipo di comando

## Skills (Opzionale)

Le skills sono directory in `~/.claude/skills/`. Ogni skill è una cartella con file markdown che definiscono comportamenti specializzati.

```bash
# Installa skills dalla community
git clone https://github.com/example/claude-skills.git ~/.claude/skills/my-skills
```

La statusline conta automaticamente le directory in `~/.claude/skills/`.
...
├── statusline-command.sh   # Script statusline (questo file)
├── .current_profile        # Profilo corrente (opzionale)
└── skills/                 # Skills directory (opzionale)
    ├── skill-1/
    ├── skill-2/
    └── ...
```

## Interpretazione della Statusline

```
claude-opus-4-6 [ios] │ 313[conda] │ ●6 MCP │ 57 skills │ 4 plugins
IN   ████████░░░░░░░░░░░░░░░░░░░░░░░░░░  81K/1M   8% │ 919K free
OUT  ████████████████░░░░░░░░░░░░░░░░░░  45K/128K 35%
SKL  ████████████████░░░░░░░░░░░░░░░░░░ ~171K/500K 34% │ 329K free
MCP  ██████████████████████░░░░░░░░░░░░  6/6 servers
TOT  ██████████░░░░░░░░░░░░░░░░░░░░░░░░  126K total 21%
  │ ●filesystem[npx] ●github[npx] ●memory[uvx]
  │ ⚙ global
```
...
|----------|-------------|
| `claude-opus-4-6` | Modello in uso |
| `[ios]` | Profilo corrente (colorato) |
| `313[conda]` | Ambiente Python (313 = nome, conda = tipo) |
| `●6 MCP` | 6 MCP servers, tutti healthy (verde) |
| `57 skills` | Numero di skills installate |
| `4 plugins` | Plugins attivi |

### Barre

| Barra | Descrizione |
|-------|-------------|
| **IN** | Token input usati / Context window totale |
| **OUT** | Token output / Limite output (128K default) |
| **SKL** | Stima token skills / Limite consigliato (500K) |
| **MCP** | MCP servers configurati (10 = 100%) |
| **TOT** | Media combinata IN + OUT |

### Colori Barre (Pastello)

| Zona | Percentuale | Colore | Significato |
|------|-------------|--------|-------------|
| Green | 0-50% | Verde pastello | Tutto OK |
| Yellow | 50-70% | Giallo pastello | Inizia a riempirsi |
| Orange | 70-85% | Arancione pastello | Attenzione |
| Red | 85-100% | Rosso pastello | Critico - usa /compact |

### Warning

- `🟡 Warning` (70%+): Il contesto si sta riempiendo
- `🔴 CRITICAL /compact` (85%+): Contesto quasi pieno, usa `/compact`

## Personalizzazione

### Cambiare colori

Modifica i codici ANSI in `build_bar()` e `build_danger_bar()`:

```bash
# Codici colore pastello
114 = Verde soft
180 = Giallo soft
173 = Arancione soft
167 = Rosso soft
238 = Grigio scuro (sfondo)
```

### Cambiare larghezza barre

Modifica il parametro width (default 36):

```bash
IN_BAR=$(build_danger_bar "$USED_PCT" 36)  # Cambia 36
```
...
### Statusline non appare

1. Verifica che lo script sia eseguibile:
   ```bash
   chmod +x ~/.claude/statusline-command.sh
   ```

2. Verifica il path in settings.json:
   ```bash
...
### MCP servers non mostrati

Verifica che `settings.json` abbia la sezione `mcpServers`:
```bash
...
Verifica la struttura directory:
```bash
ls -la ~/.claude/skills/
```

Ogni skill deve essere una directory (non un file).

---

## Codice Completo: statusline-command.sh

<details>
<summary>Clicca per espandere il codice</summary>
...
Creato per la community di Claude Code.

---

**Domande?** Apri una issue su GitHub o contattami su [Twitter/X](https://twitter.com/yourhandle).
